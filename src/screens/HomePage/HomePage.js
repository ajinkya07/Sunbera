...
class HomePage extends Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      if (stat !== 'active') {
        ...
        this.showToast(
          'User is not Active. Please contact System Admin',
          'danger',
        );
      }
    }
  ...
  render() {
    ...
    let userStatus =
      allParameterData &&
      allParameterData.splash_popup &&
      allParameterData.splash_popup[0];

    let link = urls.imageUrl + 'public/backend/collection/';

    const categoryData =
      homePageData && homePageData.collection ? homePageData.collection : [];
